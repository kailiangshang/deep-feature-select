...
class GumbelSoftmaxGateConcreteModel(GateFeatureModule):
    def __init__(
        self,
        input_dim: int,
        k: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.training:
            ...
            enc_noise = generate_gumbel_noise(self.logits_encoder)
            enc_soft_prob = F.softmax((self.logits_encoder + enc_noise) / self.temperature, dim=0)
    ...
    def sparsity_loss(self) -> SparsityLoss:
        ...
        return SparsityLoss(names=["gsg_cae_l1"], values=[l1_loss])
