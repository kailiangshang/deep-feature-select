...
class GateEncoderTrainer:
    ...
    def fit(self, train_loader, epochs, test_loader=None):
        ...
        for epoch in range(epochs):
            ...
            gate_diag = self.model.gate_diagnostics()
            enc_diag = self.model.encoder_diagnostics()
            records.append(
                {
                    "epoch": epoch + 1,
                    "loss_task": loss_task,
                    ...
                    "gate_open_ratio": gate_diag.open_ratio,
                    "gate_entropy": gate_diag.entropy,
                    "encoder_overlap": enc_diag.feature_overlap if enc_diag else 0,
                }
            )
            selected_features_list.append(sel_result.selected_indices.copy())
            print(
                ...
                f"Gate open: {gate_diag.open_ratio:.3f}"
            )
        feature_df = pd.DataFrame(
            selected_features_list,
            columns=[f"slot_{i}" for i in range(len(selected_features_list[0]))],
        )
        return pd.DataFrame(records), feature_df
