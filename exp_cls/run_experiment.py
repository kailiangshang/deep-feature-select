"""Unified experiment runner for exp_cls — supports encoder / gate / combined phases.

Usage:
  python -m exp_cls.run_experiment --config exp_cls/configs/encoder_cls.yaml --mode encoder
  python -m exp_cls.run_experiment --config exp_cls/configs/gate_cls.yaml --mode gate --skip-completed
"""
...
from exp_cls.dataset import generate_train_test_loader
from exp_cls.trainers import EncoderTrainer, GateTrainer, GateEncoderTrainer
from exp_cls.utils import MLPClassifier, MLPRegressor, AutoencoderHead, seed_all
...
def _is_completed(results_dir, group_name):
    return os.path.exists(
        os.path.join(results_dir, "per_group", f"{group_name}_meta.json")
    )


def _save_group(results_dir, group_name, result_df, feature_df, meta, gate_probs=None):
    ...
    if gate_probs is not None:
        np.save(os.path.join(gdir, f"{group_name}_gate_probs.npy"), gate_probs)
...
def run_encoder(
    ...
    for model_cfg in config.get("encoder_models", []):
        ...
        for k in model_cfg.get("k_values", [config.get("k_default", 20)]):
            ...
            if skip_completed and _is_completed(results_dir, group_name):
            ...
            head = _make_head(
                task, k, head_cfg["hidden_dim"], out_dim, training_cfg["device"]
            )
            trainer_kwargs = (
                {"input_dim": data.feature_dim} if task == "reconstruction" else {}
            )
            ...
            df = trainer.fit(
                data.train_loader, training_cfg["epochs"], data.test_loader
            )
...
def run_gate(
    ...
    for model_cfg in config.get("gate_models", []):
        ...
        for sw in model_cfg.get("sparse_loss_weights", [1.0]):
            ...
            all_results.append(df)
            _save_group(
                results_dir, group_name, df, features, meta, gate_probs=gate_probs
            )
...
def run_combined(
    ...
    for model_cfg in config.get("combined_models", []):
        ...
        for k in model_cfg.get("k_values", [config.get("k_default", 20)]):
            for sw in model_cfg.get("sparse_loss_weights", [1.0]):
                group_name = (
                    f"{model_cfg['name']}_{dataset_name}_seed{seed}_k{k}_sw{sw}"
                )
                if skip_completed and _is_completed(results_dir, group_name):
                    print(f"  SKIP {group_name}")
                    continue
                ...
                result_df, feature_df = trainer.fit(
                    data.train_loader, training_cfg["epochs"], data.test_loader
                )
                elapsed = time.time() - t0
...
def main():
    ...
    parser.add_argument(
        "--mode", choices=["encoder", "gate", "combined", "all"], default="all"
    )
    ...
    task = config["task"]
    results_dir = config.get("results_dir", "exp_cls/results")
    ...
    if all_results:
        ...
        summary = (
            final_df[final_df["epoch"] == last_epoch]
